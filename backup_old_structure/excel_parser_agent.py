        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert in Serbian construction and buil€g projects. Analyze construction specifications and provide structured categorization."
                    },
                    {
                        "role": "user", 
                        "content": prompt
...
class ExcelParserAgent:
    ...
    def __init__(self, debug: bool = False, enable_llm: bool = True, openai_api_key: Optional[str] = None):
        ...
        self.column_mappings = {
            ...
            'unit': ['j. mere', 'j.mere', 'jm', 'je€ica mere', 'je€ica', 'um', 'j. mera', 'mera', 'jed.'],
            'quantity': ['kol', 'kol.', 'količina', 'komada', 'količina', 'broj'],
            'unit_price': ['j. cena', 'j.cena', 'je€ična cena', 'cena po je€ici', 'j.c.', 'jed. cena', 'je€ična cena (€)', 'je€ična cena (rsd)'],
    ...
    def _load_excel_file(self, file_path: str) -> pd.DataFrame:
        ...
        try:
            # Try rea€g with openpyxl engine (recommended for .xlsx)
    ...
    def _extract_items(self, df: pd.DataFrame, header_row: int, column_map: Dict[str, int]) -> List[ConstructionItem]:
        ...
        # Start rea€g from row after header
        for row_idx in range(header_row + 1, len(df)):
            ...
            try:
                ...
                if unit_price and quantity and not total_price:
                    ...
                    if abs(calculated_total - total_price) > 0.01:  # Allow small roun€g differences
                        if self.debug:
    ...
    def _parse_number(self, value) -> Optional[float]:
        ...
        try:
            ...
            str_value = re.sub(r'[€$£¥₹₽€\s]+', '', str_value, flags=re.IGNORECASE)
    ...
    def export_results(self, items: List[ConstructionItem], input_file_path: str, output_path: str = None):
        ...
        for item in items:
            data.append({
                ...
                'Je€ica mere': item.unit,
                'Količina': item.quantity,
                'Je€ična cena': item.unit_price,
    ...
    def export_summary_report(self, items: List[ConstructionItem], input_file_path: str):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if items_with_prices > 0:
                f.write(f"\nFINANCIAL SUMMARY:\n")
                f.write(f"  Total value: {total_value:,.2f} EUR\n")
                f.write(f"  Items with prices: {items_with_prices}/{len(items)}\n")
                f.write(f"  Average price per item: {total_value/items_with_prices:,.2f} EUR\n")
            ...
            for i, item in enumerate(items, 1):
                ...
                if item.unit_price:
                    f.write(f"   Price: {item.unit_price:.2f} EUR/{item.unit}\n")
    ...
    def print_summary(self, items: List[ConstructionItem]):
        ...
        if items_with_prices > 0:
            print(f"\n💰 Finansijski pregled:")
            print(f"  • Ukupna vrednost: {total_value:,.2f} EUR")
            print(f"  • Stavke sa cenama: {items_with_prices}/{len(items)}")
            print(f"  • Prosečna cena po stavci: {total_value/items_with_prices:,.2f} EUR")
        ...
        for i, item in enumerate(items[:5]):
            ...
            if item.unit_price:
                print(f"     💵 {item.unit_price:.2f} EUR/{item.unit}")
...
def create_realistic_test_excel():
    ...
    # Office Buil€g Renovation - Estimated Total: €85,000
    test_data = {
        'BR.': list(range(1, len([{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}]) + 1)),
        'OPIS POZICIJE': [item['description'] for item in [{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}]],
        'J. MERE': [item['unit'] for item in [{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}]],
        'KOL.': [item['quantity'] for item in [{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}]],
    ...
    for item in [{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}]:
        category = item['category']
        price_config = {'mehanika': {'min': 150, 'max': 800, 'default': 350}, 'elektro': {'min': 80, 'max': 500, 'default': 200}, 'izolacija': {'min': 25, 'max': 150, 'default': 75}, 'montaža': {'min': 200, 'max': 1200, 'default': 600}, 'građevina': {'min': 300, 'max': 2000, 'default': 850}, 'vodovodne_instalacije': {'min': 120, 'max': 700, 'default': 300}, 'infrastruktura': {'min': 500, 'max': 5000, 'default': 1500}, 'demontaža': {'min': 50, 'max': 300, 'default': 120}, 'transport': {'min': 30, 'max': 200, 'default': 80}, 'ostalo': {'min': 100, 'max': 600, 'default': 250}}[category]
    ...
    print(f"📊 Project: Office Buil€g Renovation")
    print(f"💰 Estimated Total: €85,000")
    print(f"📋 Items: {len([{'category': 'elektro', 'description': 'LED lighting system installation', 'quantity': 45, 'unit': 'fixtures'}, {'category': 'mehanika', 'description': 'HVAC system upgrade', 'quantity': 1, 'unit': 'system'}, {'category': 'građevina', 'description': 'Interior wall modifications', 'quantity': 120, 'unit': 'm²'}, {'category': 'montaža', 'description': 'Suspended ceiling installation', 'quantity': 200, 'unit': 'm²'}, {'category': 'izolacija', 'description': 'Thermal insulation upgrade', 'quantity': 150, 'unit': 'm²'}])} construction items")
