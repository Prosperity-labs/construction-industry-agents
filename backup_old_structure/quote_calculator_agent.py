...
class QuoteCalculatorAgent:
    ...
    def calculate_quote(self, 
        ...
        logger.info(f"Quote calculated: {len(quote_items)} items, total: {final_total:,.2f} EUR")
        return calculation
    ...
    def _calculate_item_quote(self, original_item: Dict, supplier_responses: List[Dict]) -> Optional[QuoteItem]:
        ...
        quantity = float(original_item.get('quantity') or 0)
        ...
        # Calculate margins with safe numerical operations
        margin_percentage = self.category_margins.get(category, self.default_margin)
        
        # Ensure best_price is a valid number
        if best_price is None or best_price <= 0:
            logger.warning(f"Invalid best_price for item {position}: {best_price}")
            return None
    ...
    def optimize_supplier_selection(self, calculation: QuoteCalculation) -> QuoteCalculation:
        ...
        for supplier, items in supplier_groups.items():
            ...
            logger.info(f"  • {supplier}: {len(items)} items, {total:,.2f} EUR")
...
def main():
    ...
    print(f"  Subtotal: {calculation.subtotal:,.2f} EUR")
    print(f"  Final total: {calculation.final_total:,.2f} EUR")
